...
const { uploaded, setUploaded, setTotal } = require('./stdout')
...
function deImgQueue(queue, keys, compiler, compilation) {
  let reTryCount = 3
  let uploadErrorList = []
  return co(function* () {
    function* upload(fileInfo, reTryCount) {
      ...
      try {
        if (dict[fileMd5]) {
          ...
          let compressBuffer = yield new Promise(function (resolve, reject) {
            fs.readFile(
              path.resolve(__dirname, dict[fileMd5]),
              function (err, buffer) {
                if (err) {
                  reject(err)
                } else {
          ...
          compilation.updateAsset(
            fileInfo.name,
            new compiler.webpack.sources.RawSource(compressBuffer)
          )
          return
        }
      } catch (e) {
      ...
      try {
        ...
        compilation.updateAsset(
          fileInfo.name,
          new compiler.webpack.sources.RawSource(compressImg)
        )
        // 缓存压缩后的文件
        let filePath = yield writeImg(compressImg, fileMd5)
        appendDict[fileMd5] = path.relative(__dirname, filePath)
      } catch (err) {
...
/**
 * 进行图片上传主操作
 * @param  {[type]} compiler     [webpack 构建对象]
 * @param  {[type]} compilation     [webpack 构建对象]
 * @param  {[type]} options         [选项]
 * @return {Promise}
 */
module.exports = (compiler, compilation, options) => {
  //过滤文件尾缀名称
  ...
  return co(function* () {
    ...
    setUploaded(0)
    setTotal(imgQueue[0].length + imgQueue[1].length + imgQueue[2].length)
    tinify.key = _.first(keys)
    keys = _.drop(keys)
    let result = yield Promise.all([
      deImgQueue(imgQueue[0], keys, compiler, compilation),
      deImgQueue(imgQueue[1], keys, compiler, compilation),
      deImgQueue(imgQueue[2], keys, compiler, compilation),
    ])
